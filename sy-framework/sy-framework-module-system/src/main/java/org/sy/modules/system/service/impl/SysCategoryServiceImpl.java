...
import org.sy.common.util.FillRuleUtil;
import org.sy.common.util.YouBianCodeUtil;
import org.sy.common.util.oConvertUtils;
import org.sy.modules.system.entity.SysCategory;
import org.sy.modules.system.mapper.SysCategoryMapper;
import org.sy.modules.system.model.TreeSelectModel;
import org.sy.modules.system.service.ISysCategoryService;
...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements ISysCategoryService {
	...
	@Override
	public List<TreeSelectModel> queryListByCode(String pcode) throws SyFrameWorkException{
		...
		if(oConvertUtils.isNotEmpty(pcode)) {
			...
			if(list==null || list.size() ==0) {
				throw new SyFrameWorkException("该编码【"+pcode+"】不存在，请核实!");
			}
			if(list.size()>1) {
				throw new SyFrameWorkException("该编码【"+pcode+"】存在多个，请核实!");
			}
